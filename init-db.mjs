...
async function initDB() {
  try {
    ...
    // ─── NEW: Daily Check-in Feature Tables ───
    console.log('[init-db] Checking check-in feature tables...')

    if (
      await createTableIfNotExists(
        'cm8_users',
        `
      CREATE TABLE cm8_users (
        id serial PRIMARY KEY,
        phone varchar(20) UNIQUE NOT NULL,
        password_hash varchar(255) NOT NULL,
        cm8_player_id varchar(100) NOT NULL,
    ...
    `,
      ...
      createdAnything = true
      console.log('[init-db]   Created cm8_users')
    }

    // Add referral columns if missing (migration for existing installs)
    try {
      await client.query(`ALTER TABLE cm8_users ADD COLUMN IF NOT EXISTS referral_code varchar(10) UNIQUE`)
      await client.query(`ALTER TABLE cm8_users ADD COLUMN IF NOT EXISTS referred_by integer REFERENCES cm8_users(id)`)
      await client.query(`ALTER TABLE cm8_users ADD COLUMN IF NOT EXISTS referral_count integer DEFAULT 0`)
    } catch (_e) { /* columns may already exist */ }

    // Backfill referral codes for existing users without one
    try {
      await client.query(`
        UPDATE cm8_users SET referral_code = UPPER(SUBSTRING(MD5(RANDOM()::text || id::text) FROM 1 FOR 8))
    ...
    if (
      await createTableIfNotExists(
        'cm8_checkins',
        `
      CREATE TABLE cm8_checkins (
        id serial PRIMARY KEY,
        user_id integer NOT NULL REFERENCES cm8_users(id) ON DELETE CASCADE,
    ...
    `,
      ...
      createdAnything = true
      console.log('[init-db]   Created cm8_checkins')
    }
    ...
    if (
      await createTableIfNotExists(
        'cm8_spin_vouchers',
        `
      CREATE TABLE cm8_spin_vouchers (
        id serial PRIMARY KEY,
        user_id integer NOT NULL REFERENCES cm8_users(id) ON DELETE CASCADE,
        created_at timestamp(3) with time zone DEFAULT now() NOT NULL,
    ...
    `,
      ...
      createdAnything = true
      console.log('[init-db]   Created cm8_spin_vouchers')
    }
    ...
    // Media: mobile_banner size columns
    await addColumnIfNotExists('media', 'sizes_mobile_banner_url', 'varchar')
    await addColumnIfNotExists('media', 'sizes_mobile_banner_width', 'numeric')
    await addColumnIfNotExists('media', 'sizes_mobile_banner_height', 'numeric')
    await addColumnIfNotExists('media', 'sizes_mobile_banner_mime_type', 'varchar')
    await addColumnIfNotExists('media', 'sizes_mobile_banner_filesize', 'numeric')
    await addColumnIfNotExists('media', 'sizes_mobile_banner_filename', 'varchar')
    ...
    // ─── Check-in feature indexes ───
    await createIndexIfNotExists(
      'CREATE UNIQUE INDEX cm8_users_phone_idx ON cm8_users (phone)',
    )
    await createIndexIfNotExists(
      'CREATE INDEX cm8_users_created_at_idx ON cm8_users (created_at)',
    )
    await createIndexIfNotExists(
      'CREATE INDEX cm8_checkins_user_id_idx ON cm8_checkins (user_id)',
    )
    await createIndexIfNotExists(
      'CREATE INDEX cm8_checkins_checkin_date_idx ON cm8_checkins (checkin_date)',
    )
    await createIndexIfNotExists(
      'CREATE UNIQUE INDEX cm8_checkins_user_date_idx ON cm8_checkins (user_id, checkin_date)',
    )
    await createIndexIfNotExists(
      'CREATE INDEX cm8_spin_vouchers_user_id_idx ON cm8_spin_vouchers (user_id)',
    )
    await createIndexIfNotExists(
      'CREATE INDEX cm8_spin_vouchers_used_idx ON cm8_spin_vouchers (user_id, used)',
    )
