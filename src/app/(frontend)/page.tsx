...
const articleSchema = {
  ...
  publisher: {
    ...
    logo: {
      '@type': 'ImageObject',
      url: 'https://www.cm8vvip.com/cm8-logo.png',
    },
  },
  description:
...
export default async function HomePage() {
  ...
  const cmsWinGallery = await getWinGallery()
    ...
    ? (rawBanners
        .map((b) => {
          const img = b.image as
            | { url?: string; mimeType?: string; sizes?: { mobileBanner?: { url?: string } } }
            | string
            | undefined
          const uploadSrc = typeof img === 'object' && img?.url ? img.url : null
          const mobileSrc =
            typeof img === 'object' && img?.sizes?.mobileBanner?.url
              ? img.sizes.mobileBanner.url
              : undefined
          const src = (b as unknown as Record<string, string>).imageUrl || uploadSrc || null
          if (!src) return null
          const mimeType = typeof img === 'object' && img?.mimeType ? img.mimeType : undefined
  ...
  // Win Gallery from CMS
  const displayWinGallery = cmsWinGallery
    ? cmsWinGallery.map((w) => {
        const img = w.image as { url?: string } | string | undefined
        const imgUrl = typeof img === 'object' && img?.url ? img.url : null
        const withBust = (u?: string | null) => {
          if (!u) return u
          return u.includes('?') ? `${u}&v=20260227` : `${u}?v=20260227`
        }
        return {
          imageUrl: withBust(imgUrl) || '/avatars/agent-1.png',
          playerName: (w.playerName as string) || '',
          amount: (w.amount as string) || '',
          gameName: (w.gameName as string) || '',
        }
      })
    : null
  ...
  // Testimonials (hardcoded only — no longer from income CMS)
  ...
  return (
    <>
      ...
      <div className="quick-actions">
        ...
        >
          ...
          <div className="qa-text">
            <span className="quick-action-label">Winrate Scanner</span>
      ...
      <div className="section-title-bar">
        <h2>🏆 Kemenangan Pemain CM8</h2>
      </div>
      <p className="section-subtitle-bar">Bukti kemenangan sebenar pemain kami</p>
      {displayWinGallery ? (
        <div className="win-gallery-wrapper">
          <div className="win-gallery-track">
            {/* Original set */}
            {displayWinGallery.map((win, i) => (
              <div key={`a-${i}`} className="win-gallery-item">
                <Image
                  src={win.imageUrl}
                  alt={win.playerName ? `${win.playerName} - ${win.amount}` : 'Kemenangan CM8'}
                  width={270}
                  height={480}
                  className="win-gallery-img"
                  unoptimized
                />
                {(win.playerName || win.amount) && (
                  <div className="win-gallery-overlay">
                    {win.playerName && <span className="win-gallery-name">{win.playerName}</span>}
                    {win.amount && <span className="win-gallery-amount">{win.amount}</span>}
                    {win.gameName && <span className="win-gallery-game">{win.gameName}</span>}
                  </div>
                )}
              </div>
            ))}
            ...
            {displayWinGallery.map((win, i) => (
              <div key={`b-${i}`} className="win-gallery-item" aria-hidden="true">
                <Image
                  src={win.imageUrl}
                  alt=""
                  width={270}
      ...
      <div style={{ textAlign: 'center', padding: '16px 16px 24px' }}>
        ...
        >
          📱 Join WhatsApp Channel — Live Winning Games
        </a>
      </div>
      ...
      <div className="section-title-bar">
        <h2>🌍 Mata Wang Disokong</h2>
      </div>
      <p className="section-subtitle-bar">Main dari mana-mana negara dengan mata wang tempatan</p>
      <div className="currency-info-box">
        <div className="currency-info-item">
          <span className="currency-info-icon">💳</span>
          <div>
            <strong>Deposit</strong>
            <p>Point dalam game CM8 menggunakan MYR (Ringgit Malaysia)</p>
          </div>
        </div>
        <div className="currency-info-item">
          <span className="currency-info-icon">💱</span>
          <div>
            ...
            <p>Pemain luar negara — wang akan ditukar ke mata wang tempatan secara automatik</p>
          </div>
        </div>
        <div className="currency-info-item">
          <span className="currency-info-icon">🌐</span>
          <div>
            <strong>USDT (Global)</strong>
            <p>Guna USDT — sesiapa dari mana-mana negara di dunia boleh main!</p>
          </div>
        </div>
      </div>
      <div className="currency-grid">
        {[
          { flag: '🇲🇾', code: 'MYR', name: 'Malaysia' },
          { flag: '🇮🇩', code: 'IDR', name: 'Indonesia' },
          { flag: '💲', code: 'USDT', name: 'Global (Crypto)' },
          { flag: '🇧🇩', code: 'BDT', name: 'Bangladesh' },
          { flag: '🇵🇭', code: 'PHP', name: 'Philippines' },
          { flag: '🇻🇳', code: 'VND', name: 'Vietnam' },
          { flag: '🇳🇵', code: 'NPR', name: 'Nepal' },
          { flag: '🇹🇭', code: 'THB', name: 'Thailand' },
        ].map((c, i) => (
