...
export default function PatchIDPage() {
  const fallbackScannerImg = '/banners/banner-cm8-scanner.jpg?v=20260227b'
  const handleBrokenImg = useCallback((e: React.SyntheticEvent<HTMLImageElement>) => {
    const img = e.currentTarget
    if (img.dataset.fallbackApplied === '1') return
    img.dataset.fallbackApplied = '1'
    img.src = fallbackScannerImg
  }, [])
  ...
  const shareResults = useCallback(
    async (method?: 'whatsapp' | 'telegram' | 'copy') => {
      ...
      if (method === 'telegram') {
        window.open(
          `https://t.me/share/url?url=${encodeURIComponent('https://www.cm8vvip.com/patch-id')}&text=${encodeURIComponent(text)}`,
          '_blank',
        )
        return
      }
